...
.login-apple{background:#111;color:#fff;gap:12px}
.login-soon{background:#d8dee3;color:#5c6b73;gap:12px;cursor:not-allowed}
.login-social-lead{margin:0 0 10px;font-size:18px;font-weight:700;color:var(--deep)}
.apple-relay-note{margin:8px 0 0;font-size:16px;line-height:1.6;color:#5a6e78}
...
<body data-member-api="<?php echo is_file(__DIR__ . '/pm_member_api_v17.php') ? 'pm_member_api_v17.php' : 'pm_member_api_v18.php'; ?>" data-portal="<?php echo htmlspecialchars(basename($_SERVER['SCRIPT_NAME'] ?? 'pm_member_portal_v17.php'), ENT_QUOTES, 'UTF-8'); ?>">
...
<section id="guestView" class="hidden" aria-label="登入或註冊">
  <div class="login-card" id="loginCard">
    ...
    <p class="login-social-lead">用這個帳號快速登入</p>
    <div class="social-row" id="socialRow">
      <button type="button" class="login-google" data-provider="google">
      ...
      <button type="button" class="login-line" data-provider="line">
      ...
      <button type="button" class="login-apple" data-provider="apple" id="appleSignIn">
        ...
        <span data-label>使用 Apple 繼續</span>
      </button>
    </div>
    <p class="apple-relay-note">若開啟隱藏信箱，我們會以手機聯絡您。</p>
...
document.getElementById('logoutBtn').onclick=async()=>{try{
  await call('logout');
  location.replace((window.PM_MEMBER_PORTAL||'pm_member_portal_v17.php')+'?logout=1&t='+Date.now());
