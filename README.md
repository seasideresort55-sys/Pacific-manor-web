...
正式會員入口覆蓋官網同名檔（手機簡訊主路徑 + 可點的 Apple），不再使用 `#socialLogin` 當公開入口：

https://seasideresort.com.tw/booking/pm_roomboard/pm_member_portal_v17.php

可上傳檔在 [`deploy/`](deploy/README.md)。客人看到的驗證碼失敗只會是「暫時無法寄送驗證碼，請稍後再試或改用其他方式」。主機金鑰與 Email UNIQUE／身分綁定衝突見 [`deploy/OPERATOR_OTP.md`](deploy/OPERATOR_OTP.md)；本包不擅自 ADD／DROP 索引。
...
4c. 不另造會員庫。驗證後以手機號碼作為與 [pm_member_portal v17](https://seasideresort.com.tw/booking/pm_roomboard/pm_member_portal_v17.php)／`qlo_pm_member` 同一識別。Apple 只認 `apple_sub`（對齊 PR #3 IDENTITY），不把 Email 當主鍵。正式 PHP 已補 `phone_login_request`／`phone_login_verify`；登入後綁定仍走 `phone_request`／`phone_verify`。
